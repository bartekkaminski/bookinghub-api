...
WORKDIR /src
COPY ["BookingHub.Api.csproj", "."]
RUN dotnet restore "./BookingHub.Api.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./BookingHub.Api.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./BookingHub.Api.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
